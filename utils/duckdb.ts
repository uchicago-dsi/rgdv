/**
 * Adapated from https://github.com/ryan-williams/next-duckdb-parquet-demo/blob/main/src/parquet.ts
 */
/**
...
export async function runQuery<T>({
  ...
  if (!db && !conn) {
    throw Error("runQuery: need either db or conn")
  }
  const _conn = conn ? conn : await db!.connect()
...
export async function parquetBuf2json<T>(bytes: number[] | Uint8Array, table: string): Promise<T[]> {
  ...
  return runQuery({
    db,
    query: `SELECT * FROM parquet_scan('${table}')`,
  })
}
